...
const RecuerdaMed = ({ session, modoOscuro, setModoOscuro }) => {
  ...
  return (
    <div className={`min-h-screen p-6 md:p-12 font-sans transition-colors duration-300 ${
      ...
      <header className={`mb-8 flex flex-col md:flex-row justify-between items-center gap-4 border-b pb-6 transition-colors ${modoOscuro ? 'border-gray-700' : 'border-gray-200'}`}>
        <div className="text-center md:text-left">
          <h1 className={`text-4xl font-extrabold tracking-tight transition-colors ${modoOscuro ? 'text-white' : 'text-gray-900'}`}>
            RecuerdaMed
          </h1>
          <p className={`mt-2 text-lg transition-colors ${modoOscuro ? 'text-gray-300' : 'text-gray-600'}`}>
            {session.user.email}
          </p>
        </div>
        <button 
      ...
      {vista === 'inicio' && (
        <div className="flex flex-col items-center w-full max-w-4xl mx-auto mt-8 space-y-8 fade-in">
          <h2 className={`text-3xl font-extrabold transition-colors ${modoOscuro ? 'text-white' : 'text-gray-800'}`}>
            ¿Qué deseas gestionar hoy?
          </h2>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-16 w-half">
            <CardTemplate
      ...
      {vista === 'med' && (
        <div className="fade-in">
          <button onClick={() => setVista('inicio')} className={`mb-6 text-sm font-bold underline transition-colors cursor-pointer ${modoOscuro ? 'text-green-400 hover:text-green-300' : 'text-green-700 hover:text-green-800'}`}>
      ...
      {vista === 'citas' && (
        <div className="fade-in">
          ...
          <div className={`p-8 rounded-xl shadow-md border text-center transition-colors ${modoOscuro ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'}`}>
